...
[^3]: Some checks will always remain dynamic/runtime. For instance, think about the "API boundary": a client must parse and then typecheck a string/JSON value. A static tool doesn't add any guarantee there, the case, and how to continue/recover when things go wrong, must be handled at application/runtime level, ever.