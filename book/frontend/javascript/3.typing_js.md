...
We use [tcomb](https://github.com/gcanti/tcomb) for *runtime* type checking, and [Flow](https://github.com/facebook/flow)/[TypeScript](https://github.com/Microsoft/TypeScript) for *static* type checking.
...
`babel-plugin-tcomb` is particularly interesting as a first step in the static type checking direction: provided a way to make, for example, flow understand our tcomb annotations[^2] (or viceversa, translating the annotations to flow and do the opposite), we should be able to get the best of both worlds.[^3]
